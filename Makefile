...
coverage:
	@echo "STATIC LIBRARY"
	@cd build && cmake .. -DASYNC=OFF -DTEST=ON > cmakelog.txt && make > makelog.txt && ./tests/dz2_test 
	@cd build && lcov -t "tests/test_dz2" -o coverage.info -c -d dz2s/ && genhtml -o report coverage.info > static_cov.txt
	@echo "ASYNC LIBRARY"
	@cd build && cmake .. -DASYNC=ON -DTEST=ON > cmakelog.txt && make > makelog.txt && ./tests/dz2_test
	@cd build && lcov -t "tests/test_dz2" -o coverage.info -c -d dz2d/ && genhtml -o report coverage.info > async_cov.txt
